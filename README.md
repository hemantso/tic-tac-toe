...
2. Encapsulate Tic Tac Toe in a `Game` object.

## Overview

You're going to be building a 2 player CLI version of Tic Tac Toe by building a `Game` object. The game play will be very similar to other versions of TicTacToe.
...
Our main `Game` class will be defined here with all the data and logic required to play a game of tic tac toe via instances of `Game`.
...
We'll be following the Tic Tac Toe conventions of representing the board as an array with 9 elements represents an empty cells in the board.
...
Open up `./bin/main.rb`. You'll be defining the main game class, `Game` in `bin/main.rb`. Without that file defining a `Game` class, everything will break.

Every method you build will be encapsulated by this class.

### `#initialize` and `@board`

The first test in `main.rb` will ensure the requirement that when a new game of Tic Tac Toe is started — that is, when a new instance of `Gamee` is initialized — the instance of the game must set the starting state of the board, an array with 9 `" "` empty strings, within an instance variable named `@board`.
...
# the rest of the Game class definition
```
